...
class Sol(ProviderParser):
    ...
    @staticmethod
    def parse_latest_shows() -> Iterator[List[Film]]:
    ...
    @staticmethod
    def parse_latest_movies() -> Iterator[List[Film]]:
        home_url: str = f"{Sol.host_url}/home"
        r: requests.Response = requests.get(home_url,headers=Sol.headers)
        html_doc : lxml.html.HtmlElement = lxml.html.fromstring(r.text)
        section_elements : List[lxml.html.HtmlElement] = html_doc.cssselect(".section-id-02")
    ...
    @staticmethod
    def parse_latest() -> Iterator[List[Film]]:
        tv_shows_generator: Iterator[List[Film]] = Sol.parse_latest_shows()
        movies_generator: Iterator[List[Film]] = Sol.parse_latest_movies()
    ...
    @staticmethod
    def parse_trending_movies() -> Iterator[List[Film]]:
        home_url: str = f"{Sol.host_url}/home"
        r: requests.Response = requests.get(home_url,headers=Sol.headers)
        html_doc : lxml.html.HtmlElement = lxml.html.fromstring(r.text)
    ...
    @staticmethod
    def parse_trending_shows() -> Iterator[List[Film]]:
        home_url: str = f"{Sol.host_url}/home"
        r: requests.Response = requests.get(home_url,headers=Sol.headers)
        html_doc : lxml.html.HtmlElement = lxml.html.fromstring(r.text)
    ...
    @staticmethod
    def parse_trending() -> Iterator[List[Film]]:
        tv_shows_generator: Iterator[List[Film]] = Sol.parse_trending_shows()
        movies_generator: Iterator[List[Film]] = Sol.parse_trending_movies()
        more_shows: bool = True
        more_movies: bool = True
        while(more_shows or more_movies):
            films: List[Film] = []
    ...
    @staticmethod
    def parse_coming_soon() -> Iterator[List[Film]]:
        home_url: str = f"{Sol.host_url}/home"
        r: requests.Response = requests.get(home_url,headers=Sol.headers)
        html_doc : lxml.html.HtmlElement = lxml.html.fromstring(r.text)
        section_elements : List[lxml.html.HtmlElement] = html_doc.cssselect(".section-id-02")
    ...
    def parse_category(category: str) ->Iterator[List[Film]]:
        if(category == "latest"):
            #latest_generator = Sol.parse_latest()
            return Sol.parse_latest()
            #for latest in latest_generator:
            #    yield latest

        if(category == "trending"):
            #trending_generator = Sol.parse_trending()
            #for trending in trending_generator:
            #    yield trending
            return Sol.parse_trending()

        if(category == "coming soon"):
            #coming_soon_generator = Sol.parse_coming_soon()
            #for coming_soon in coming_soon_generator:
            #    yield coming_soon
