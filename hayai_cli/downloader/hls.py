#credit goes to the owner of this github at https://github.com/justfoolingaround/animdl/blob/master/animdl/core/codebase/downloader/hls.py
import logging
import httpx
...
from hayai_cli.utilities import intelliq
...
def hls_yield(session, q_dicts, quality_string, auto_retry=5, *, continuation_index=1):
    ...
    if encryption_state:
        decryptor = get_decrypter(
            encryption_data,
            iv=encryption_iv or None,#pyright: ignore
    ...
    for current_count, stream in enumerate(stream_iter, continuation_index):
        ...
        if encryption_state:
            ts_data = decryptor(ts_data) #pyright: ignore
